...
def keras_epochs_higher(train, test):
    '''Implements an LSTM model with a higher number of epochs. May attempt to use a pretrained model.
    ...
    print('\nLSTM Model with a higher number of epochs')
    ...
    if ((saved == 'yes' or saved == 'Yes' or saved == 'y' or saved == 'YES') and os.path.isdir('models/epochs_higher_model')):
        # attempt running a pretrained model
        model = load_model('models/epochs_higher_model')
    else:
